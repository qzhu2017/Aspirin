...
for i, l in enumerate(lines[3:]):
    ...
    if len(tmp) == 2:
        ...
        for cell in range(int(N_mols/4)): 
            tmp0 = np.array([float(t) for t in lines[i+4+cell*4+0].split()[1:]])
            tmp1 = np.array([float(t) for t in lines[i+4+cell*4+1].split()[1:]])
            tmp2 = np.array([float(t) for t in lines[i+4+cell*4+2].split()[1:]])
            tmp3 = np.array([float(t) for t in lines[i+4+cell*4+3].split()[1:]])
            pos[cell*2+0] += (tmp0[:3]+tmp1[:3])/2
            pos[cell*2+1] += (tmp2[:3]+tmp3[:3])/2
            vel[cell*2+0] += (tmp0[3:]+tmp1[3:])/2
            vel[cell*2+1] += (tmp2[3:]+tmp3[3:])/2
